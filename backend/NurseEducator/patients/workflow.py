...
from .forms import PatientDiscontinuationForm, PatientRejectionForm, PatientProgramForm, PatientProgram
...
class PatientEnrollmentWorkflow(WorkflowBase):
    status_transitions = [
        ...
        {
            "name": "apply_program",
            "display_name": "Apply Program",
            "description": "Apply Program",
            "confirmation_message": "Are you sure you want to apply the program for this patient?",
            "from": "enrolled",
            "to": "program_applied",
            "form": PatientProgramForm
        }
    ]
    ...
    def apply_program_condition(self, request, object_instance, **kwargs):
        exec_role = UserRoleModel.objects.get(name='Executive')
        has_perm = exec_role.id in request.user.roles.all().values_list('id', flat=True)
        program_exists = PatientProgram.objects.filter(patient=object_instance).exists()
        if has_perm and not program_exists: 
            return True if kwargs.get('current_status') == 'approved' else False
        return False

    def apply_program_done(self, request, object_instance, transaction_obj, **kwargs):
        pass
...
class PatientProgramEnrollmentWorkflow(WorkflowBase):
    ...
    def pending_approval_to_approved_condition(self, request, object_instance, **kwargs):
        exec_role = UserRoleModel.objects.get(name='Executive')
        has_perm = exec_role.id in request.user.roles.all().values_list('id', flat=True)
        return True if has_perm and kwargs.get('current_status') == 'pending_approval' else False
    ...
    def rejected_to_approved_condition(self, request, object_instance, **kwargs):
        exec_role = UserRoleModel.objects.get(name='Executive')
        has_perm = exec_role.id in request.user.roles.all().values_list('id', flat=True)
        return True if has_perm and kwargs.get('current_status') == 'rejected' else False
    ...
    def approved_to_discontinued_condition(self, request, object_instance, **kwargs):
        exec_role = UserRoleModel.objects.get(name='Executive')
        has_perm = exec_role.id in request.user.roles.all().values_list('id', flat=True)
        return True if has_perm and kwargs.get('current_status') == 'approved' else False
