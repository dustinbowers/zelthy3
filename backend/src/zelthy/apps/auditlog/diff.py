...
def track_field(field):
    ...
    Untracked fields are many-to-many relations and relations to the Auditlog ObjectStore model.
    ...
    # Do not track relations to ObjectStore
    if (
        getattr(field, "remote_field", None) is not None
        and field.remote_field.model == ObjectStore
    ):
        return False

    # 1.8 check
    elif getattr(field, "rel", None) is not None and field.rel.to == ObjectStore:
        return False
